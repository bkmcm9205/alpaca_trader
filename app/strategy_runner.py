...
# model preload (force load + clear diagnostics on boot)
try:
    from app.strategies.model_runtime import maybe_load_model
except Exception:
    maybe_load_model = None  # if not available we’ll just skip preloading
...
# =============================
#     MODEL PRELOAD AT BOOT
# =============================

def _preload_models(active_names: List[str]):
    if maybe_load_model is None:
        print("[BOOT] Model preload skipped (no model_runtime.maybe_load_model)", flush=True)
        return
    print(f"[BOOT] Preloading models for: {active_names}", flush=True)
    for strat in active_names:
        try:
            obj = maybe_load_model(strat)
            if obj is None:
                print(f"[MODEL-PRELOAD:{strat}] no model found (override/local/S3).", flush=True)
            else:
                print(f"[MODEL-PRELOAD:{strat}] ready.", flush=True)
        except Exception as e:
            print(f"[MODEL-PRELOAD:{strat}] error: {e}", flush=True)
...
def scan_once(adapters) -> List[Candidate]:
    ...
    for sym in symbols:
        ...
        try:
            ...
            # network/HTTP-layer exceptions—skip symbol
            continue
...
def main():
    ...
    # Force a single model load + diagnostic per active strategy right at boot
    _preload_models(list(adapters.keys()))
