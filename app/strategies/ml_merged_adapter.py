...
from app.strategies.ml_merged import compute_signal as _compute  # type: ignore
...
def decide(symbol: str, df1m: pd.DataFrame, tf_minutes: int) -> dict | None:
    ...
    try:
        sig = _compute("ml_pattern", symbol, tf_minutes, df1m=df1m, model=_model_blob, sentiment="neutral")
    except TypeError:
        sig = _compute("ml_pattern", symbol, tf_minutes, df1m=df1m, sentiment="neutral")
    if not sig:
        return None
